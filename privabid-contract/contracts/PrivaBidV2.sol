...
contract PrivaBidV2 {
    ...
    /// @dev Last sealed amount per participant (bid / ask / threshold) for decryptForView.
    mapping(address => euint64) private mySealedAmount;
    ...
    function setThreshold(uint64 threshold) external whileActive {
        ...
        FHE.allowSender(encThreshold);

        dutchThresholds[msg.sender] = encThreshold;
        mySealedAmount[msg.sender] = encThreshold;
     ...
     */
    /**
     * @notice Before Dutch reveal: allow Threshold decrypt of the winner's threshold
     *         (Dutch thresholds are not made public at closeBidding).
     */
    function authorizeDutchWinnerReveal(address winner) external onlyAuctioneer whenClosed {
        ...
        FHE.allowPublic(dutchThresholds[winner]);
    }
    ...
    /// @notice Encrypted handle for msg.sender's last bid / ask / threshold (decryptForView).
    function getMySealedAmountHandle() external view returns (euint64) {
        require(hasBid[msg.sender], "No sealed amount for this address");
        return mySealedAmount[msg.sender];
    }
